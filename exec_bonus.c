...
/*   Updated: 2025/10/13 01:11:53 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    setup_infile(struct s_shared *shared, char **av, int i)
{
    ...
    else
    {
        ...
        if (infile < 0)
        {
            ...
            free_pipes(shared);  // Libérer la mémoire
            perror("open infile");
            exit(ERROR);
        }
...
void    prepare_and_exec_command(struct s_shared *shared, char **av, int i)
{
    ...
    // Libérer les pipes avant execve ou exit
    free_pipes(shared);
    
    if (!absolute_path)  // Commande introuvable
    {
        ft_putstr_fd(cmd[0], 2);
        ft_putstr_fd(": command not found\n", 2);
        clean_and_exit(cmd, NULL, 127);  // Code 127 = command not found
    }
...
int execute_commands(struct s_shared *shared, char **av, int i)
{
    ...
    if (child_pid == 0)
    {
        // Libérer pid immédiatement dans l'enfant
        free(shared->pid);
